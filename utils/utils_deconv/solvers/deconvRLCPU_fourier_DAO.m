...
%ELi, 20220531, final
function [Xguess,shifts_estimated] = deconvRLCPU_fourier_DAO(psf_all,proj_all,PSFParameters,maxIter,maxIter_init,AO,Xguess,updateWeight,savepath,matSaveOn,tifSaveOn,sidelobe)
...
Xguess=(single(Xguess)); largepsf = zeros(img_r,img_c,psf_s,'single');
...
for iterNow = 1:maxIter_init
    for i = 1:length(initializeXguessAnglesIndex)
        %%%preparations
        largepsf((img_r-psf_r)/2+1:(img_r+psf_r)/2,(img_c-psf_c)/2+1:(img_c+psf_c)/2,:) = (squeeze(psf_all(:,:,:,initializeXguessAnglesIndex(i))));
        ...
        errorBack = backwardProjectRLCPU_fourier(largepsf,HXguessFFT,psf_s,img_r,img_c,projHere);
...
for iterNow = 1:maxIter
    ...
    for angleNow = psf_iterSequence
        if updateWeight(angleNow)==0
            ...
            %%%preparations
            largepsf((img_r-psf_r)/2+1:(img_r+psf_r)/2,(img_c-psf_c)/2+1:(img_c+psf_c)/2,:)=(squeeze(psf_all(:,:,:,angleNow)));
...
%         disp('Estimating shifts...');
        for angleNow = psf_iterSequence
            ...
            sub_proj=(squeeze(proj_all(yRangeForCorr,xRangeForCorr,angleNow)));
